...
int open_serial_socket() {
    ...
    /* Inverse UART lines if needed */
    if (DB_UART_TX_INV) ESP_ERROR_CHECK(uart_set_line_inverse(UART_NUM_2, UART_SIGNAL_TXD_INV));
    if (DB_UART_RX_INV) ESP_ERROR_CHECK(uart_set_line_inverse(UART_NUM_2, UART_SIGNAL_RXD_INV));
